...
POST_REQUIRE_KEY_IN_URL = False  # keep False so POST URL has no key

# When posting WITHOUT key in URL, place key into payload under this field name (or set to None)
POST_KEY_FIELD_NAME: str | None = "PLANT"

# Where to fetch CSRF before POST/PATCH/DELETE:
#   "service"    -> /Service/       (root)  [preferred]
...
# Include ?sap-client on the CSRF probe (flip to False if your system dislikes it on probe)
INCLUDE_CLIENT_ON_CSRF = True

# On 403 with x-csrf-token: Required, auto-refetch and retry once
RETRY_ON_403_CSRF = True

# Wrap responses in a standard envelope
RETURN_WRAPPED = True

# Retry policy for HTTP errors like 429/5xx
RETRY_ATTEMPTS = 3
...
async def _fetch_csrf_once(
    ...
    """Fetch CSRF token once from service root OR entityset; return token or None."""
    ...
    headers = {
        **_auth_headers(),
        "X-CSRF-Token": "Fetch",
        "Accept": "application/json",
        "X-Requested-With": "XMLHttpRequest",   # important on some gateways
    }
...
async def _fetch_csrf(
    ...
    """
    Strategy:
      1) Try configured source (service root or entityset)
      2) If empty, auto-fallback to the other one
    """
    # First try configured
    ...
    # No token at all — still return empty to let server indicate Required
    return ""
...
# -------------------------------------------------
# Internal write helper with one CSRF retry if needed
# -------------------------------------------------
...
async def _write_with_csrf_retry(
    ...
    """
    Perform a write (POST/PATCH/MERGE/DELETE) and if we get 403 with
    x-csrf-token: Required, refetch CSRF and retry once automatically.
    """
    # First attempt
    ...
    if RETRY_ON_403_CSRF and r.status_code == 403:
        ...
        if need and need.lower() == "required":
            ...
            # Refetch token (same client to keep session cookies)
...
class SAPClient:
    ...
    with:
        ...
        - CSRF from service root (fallback to entityset) with X-Requested-With
        - POST to EntitySet (no key in URL) UNLESS POST_REQUIRE_KEY_IN_URL=True
        - PATCH/MERGE to Entity('key')
        - Automatic one-time CSRF retry on 403 Required
    ...
    async def call(
        ...
        async with httpx.AsyncClient(timeout=60.0, follow_redirects=True) as client:
            try:
                ...
                # State-changing → CSRF (service root first, fallback to entityset)
