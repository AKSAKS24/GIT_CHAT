...
CSRF_FETCH_FROM = "service"  # <<< changed default to service root

# Use this query parameter name for client
# If your gateway expects plain 'sap-client', change to "sap-client"
...
def _service_root_url(service_name: str) -> str:
    """
    http://host:port/sap/opu/odata/sap/<SERVICE>/?$sap-client=100
...
async def _fetch_csrf(client: httpx.AsyncClient, service_name: str, entity_set: str) -> Dict[str, Any]:
    ...
    if CSRF_FETCH_FROM == "service":
        probe = _service_root_url(service_name)  # <<< now ALWAYS service root
    else:
...
class SAPClient:
    ...
    with:
        ...
        - CSRF for POST/PATCH/DELETE (now fetched from service root)
    ...
    async def call(
        ...
        async with httpx.AsyncClient(timeout=60.0, follow_redirects=True) as client:
            try:
                if m == "GET":
                    # generic list/read from EntitySet root
                ...
                # State-changing → CSRF (SERVICE ROOT as requested)
